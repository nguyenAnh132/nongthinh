...
public class HideDiseaseUseCaseImpl implements HideDiseaseUseCase {
    ...
    public DiseaseView execute(UUID id) {
        ...
        support.requireStatus(disease, ReviewStatus.APPROVED);
        CurrentUser currentUser = currentUserProvider.getCurrentUser();
        Instant now = clockProvider.now();

        disease.hide(currentUser.getUserId(), now);
        Disease saved = diseaseRepository.save(disease);
        support.recordHistory(
                saved,
                DiseaseReviewAction.HIDDEN,
