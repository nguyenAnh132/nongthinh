...
class UpdateDiseaseUseCaseImplTest {
    ...
    private final UpdateDiseaseUseCaseImpl useCase = new UpdateDiseaseUseCaseImpl(
            support,
            repository,
            clockProvider,
            currentUserProvider
    ...
    @Test
    void brandCanUpdateApprovedDiseaseAndReturnsItToDraft() {
        Disease disease = approvedBrandDisease();
        CurrentUser brand = brandUser();
        DiseaseUpdateCommand command = new DiseaseUpdateCommand(
                "Đạo ôn lúa cập nhật",
                "dao-on-lua-cap-nhat",
                "Magnaporthe oryzae",
                CROP_TYPE_ID,
        ...
        when(repository.existsBySlugAndCropTypeId(command.slug(), CROP_TYPE_ID)).thenReturn(false);
        when(repository.save(disease)).thenReturn(disease);

        var result = useCase.execute(DISEASE_ID, command);

        assertEquals("Đạo ôn lúa cập nhật", result.name());
        assertEquals(ReviewStatus.DRAFT, result.reviewStatus());
        assertEquals(NOW, result.updatedAt());
        verify(support).requireActiveCropType(CROP_TYPE_ID);
