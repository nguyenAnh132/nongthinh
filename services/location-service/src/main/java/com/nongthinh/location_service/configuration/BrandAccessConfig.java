package com.nongthinh.location_service.configuration;
...
public class BrandAccessConfig implements WebMvcConfigurer, HandlerInterceptor {
    ...
    private boolean isRegistrationRequest(HttpServletRequest request, String path, String method, BrandAccessView access) {
        // Address labels and selectors are needed to view and complete the registration profile.
        return "GET".equals(method) && ("/provinces".equals(path)
                || path.matches("/provinces/[^/]+(/communes)?") || path.matches("/communes/[^/]+"));
    }
}
